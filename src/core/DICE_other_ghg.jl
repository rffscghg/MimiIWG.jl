...
function _get_marginal_gas_model(scen::scenario_choice, gas::Symbol, year::Int)
    ...
    set_param!(m, :gas, :E_CH4A, E_CH4A_all[:, scen_num])
    set_param!(m, :gas, :E_N2OA, E_N2OA_all[:, scen_num])

    mm = create_marginal_model(m)

    m2 = mm.marginal
    if gas == :CH4
        pulse_E = E_CH4A_all[:, scen_num]
        pulse_E[year_index] = pulse_E[year_index] + 1.
        set_param!(m2, :gas, :E_CH4A, pulse_E)
    else
        pulse_E = E_N2OA_all[:, scen_num]
        pulse_E[year_index] = pulse_E[year_index] + 1.
...
function _get_additional_forcing(scen::scenario_choice, gas::Symbol, year::Int)

    mm = _get_marginal_gas_model(scen, gas, year)
    run(mm)
